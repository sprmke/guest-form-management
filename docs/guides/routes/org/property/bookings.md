...
| Card     | `?view=card`      | Mobile default. Phone list rows are dense: guest name + amount, status badge + stay dates (`Nn` / `Ng` shorthand), optional property label / flags only when present. Email is omitted on phone (available on the booking detail). `sm+` keeps the taller stacked card with email and footer flags.                                                                                                                                                            |
