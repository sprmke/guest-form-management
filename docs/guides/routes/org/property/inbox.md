...
- Q: How do I send my guest their approved GAF, a calendar link, or a link to check their security deposit refund?
  A: Tap the **Share** icon (next to Quick reply and Suggest) in the composer. Pick a property link (Property page, Calendar, Chat with host) or search for the guest's booking to insert Stay Guide, Approved GAF, Approved Pet Form, Parking Endorsement, **Use your parking** / **Find parking** (own-default when available), Security Deposit Refund, or Leave a Review links — whichever apply to that booking's current status and documents.
