...
| Files      | `DocumentsPanel`                                        | Uploaded assets in labeled groups (Payments, Guests, Parking, Pets, Approvals, Booking) — empty groups hidden. AI verdict marks only on files the job actually checks: downpayment / balance / parking-payment receipts and every guest Valid ID (including guests 2–5). No mark on parking endorsement, pet photo, vaccination, approved GAF/pet, SD refund receipt, or booking PDF. Primary ID is listed once as **Valid ID** (not duplicated as Primary valid ID). |
...
The kanban workflow dialog (`variant="modal"`) keeps its existing shape for **card clicks**: no deck, no map modal, sticky action footer. **Column drops** skip the guest-summary / Proceed shell and open the transition confirm directly (or the required sub-form sheet first).
...
- **Pricing** — one Gemini call on the downpayment receipt, extracting amount and flagging total-vs-required mismatches. Guest-balance receipt issues (`invalid` / `unclear`, or missing at check-in) are appended from the stored `balance_receipt_ai_*` verdict — no second Gemini call. Uploading a balance receipt patches this section in place so the tab does not go Outdated.
