...
| Group         | Items                                                                                                                                                                                                   |
| ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `edit`        | **Add/Edit parking**, **Add/Edit pets** (open edit mode on that tab), **Set up parking** / **Open parking link**                                                                                        |
| `guest-links` | **Open stay guide**, **Copy stay guide link** — only once the token exists; **Open parking link**, **Copy parking link** — only when `need_parking` (Phase 7 host-assist, `useBookingParkingShareLink`) |
...
**Parking share link** (`useBookingParkingShareLink`, Phase 7): simpler than the stay-guide hook — no per-booking token to mint. The guest links this stay to a marketplace parking booking themselves once signed in (`list-linkable-property-bookings` resolves eligibility from their own identity, not a URL parameter), so the link is just the marketplace's public `/parkings` root. The two menu rows appear whenever `need_parking` is true — for a host who wants to proactively hand a guest the link instead of waiting for them to self-serve from the confirmation page/email/reminder.
...
- **Advance mode** — an **Auto** / **Manual** pill on the **right of each rail card title** (`WorkflowSubFormCard` / `WorkflowAdvanceModeBadge`, tooltip on hover/focus) and on the **Booking Workflow** map (`WorkflowAdvanceModeMark`, no tooltip — the guide line explains). Mapping: Pending Review / Ready for Check-in / Pending SD Refund = Manual; Pending Documents / Ready for Check-out = Auto (Ready for Check-out is Manual when SD is 0); nested `email-listener` = Auto; nested manual docs and parking = Manual. Not shown on the stage name, track dots, or nested tabs.
...
- Automation triggers (collapsible) — shown only on the **live** workflow step (hidden while browsing earlier stages, and hidden when status is **Completed** or **Cancelled**). **Ready for Check-in:** **Run check-out automation** (`POST sd-refund-cron` with `{ bookingId }` — this booking only). Help copy uses the property’s **SD refund email lead** (`sd_refund_cron_email_lead_minutes`), not a hardcoded two hours. The scheduled cron still scans other ready-for-check-in stays on its own; this button does not. **Ready for Check-out:** **Resend Check-out Instructions email** (resend only; does not move to the next step). The resend is not offered on Ready for Check-in.
...
**Plan gating:** Viewing prior results is free. **Run checks** / **Recheck** require **`aiValidations`** — client pre-flight in `BookingAiSummaryPanel`; server enforces on **`booking-ai-review`** and **`validate-booking-receipts`**.
...
**Set up parking** (header / mobile summary) opens `PayParkingModal` for late/at-checkin parking guests, or navigates straight to the guest pay-parking page (`buildPayParkingPath`) when parking was already availed (`hasPayParkingAvailed`). Modal title: **Parking link** (view) or **Set up parking** (create). Guest-facing legacy form tab title: **Pay parking**.
