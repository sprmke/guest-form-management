...
**Archived templates & categories:** Design and Video sidebars include an **Archived** category chip (always last; cannot be renamed or archived). Presets and builtin categories archived via **⋮ → Archive** appear there with **Unarchive**. Calendar designer presets and category sections have **no Archive menu** — built-in templates always stay visible. Saved custom calendar templates (under **Custom**, excluding **Blank calendar**) expose **⋮ → Rename** (updates all related Square / Portrait / Landscape rows that share the same AI generation or custom name) and **⋮ → Remove**, which permanently deletes those related rows (with confirmation). Preferences for Design/Video archive state persist in **localStorage** per property (`marketing-catalog:*`).
...
**AI calendar templates:** Calendar sidebar **Generate with AI** opens a modal (bottom sheet below `lg`): **Look** prompt; **Suggestions** (4 featured by default, **View more** reveals all 12 pastel themes with mini palette previews) that fill a full Instagrammable prompt; **Layout / Type / Background** visual pickers (thumbnail + short hint; `auto` or locked). Collapsed **Show advanced settings**: **Show on calendar** switches (property name, month & year, month arrows, subtitle, weekdays, legend — applied client-side after compile) and **Use when generating** (property photo / amenities / availability — unavailable options disabled). Header copy: creates Square / Portrait / Landscape, editable after. Requires a non-empty prompt (client + edge). `generate-marketing-template` POST — `{ contentType: 'calendar', prompt, includeContext?, amenitiesText?, availabilityText?, preferences? }` → schema-constrained tokens. Host preferences fold into the model prompt; the client also **locks** non-`auto` layout/font/background on the returned tokens, then compiles via `lib/calendarAiTokens.ts` (palette sanitize + WCAG-readable day numbers / headers / legend). Saves **three** custom `marketing_templates` rows (Square / Portrait / Landscape) sharing one `aiGenerationId` — one AI call. Removing any format deletes the whole set. Partial save failures toast `Saved N of 3`. Results appear under **Custom**. Design/Video AI generate is not wired yet.
