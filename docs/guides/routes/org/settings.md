...
| Brand color       | `organizations.settings.brandColor`                     | Optional hex `#RRGGBB`; defaults to `#24a88e` when unset. Tints **org-scoped admin pages** only (org selector, org dashboard, org settings, org properties). Property guest pages and property admin use per-property `app_settings.brand_color` (falls back here when unset). |
...
| **Google Calendar / Sheets**     | **Not touched.** Events and sheet rows created for properties stay in Google; only DB integration IDs are removed.                                                                                              |
