...
- `/org/:orgSlug/plans` — org shell (PayMongo checkout runs here)
- `/org/:orgSlug/property/:propertySlug/plans` — property shell (same UI; **Continue to payment** hands off to the org route)
...
**Billing is org-level only** (one `org_subscriptions` row covers enrolled properties), but hosts can **browse** Plans & Billing from either the org or property sidebar without leaving their current context. Property `/plans` renders the same page content (`OrgPlansPage` with `paidCheckoutMode="redirect-to-org"`). Choosing a tier still opens `PlanReviewDialog` in place; **Continue to payment** navigates to `/org/:orgSlug/plans?reviewPlan=<planId>` where PayMongo checkout runs. Downgrades apply immediately from either surface (no payment step).
...
**Page title:** org → `${Org Name} - Plans & Billing`; property → `${Property Name} - Plans & Billing`. Subtitle: `PLANS_PAGE_SUBTITLE` in `planPresentation.ts`.
...
- **Property sidebar:** **Plans & Billing** links to `/org/:orgSlug/property/:propertySlug/plans` (stays in property context). Org sidebar still links to `/org/:orgSlug/plans`.
...
| **Pro**      | `growth`      | ₱999 → **₱799**            | 20% off list · 5 pooled seats, **Marketing Content Studio**, top-30 search, AI validation, **Public pages editor**, **Property showcase & stay guide access**, **Airbnb & OTA calendar sync**, 1k pooled AI credits |
...
| Layer                 | Path                                                                                                                                                                                                                                                 |
| --------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Page (org)            | `ui/src/features/dashboard/plans/pages/OrgPlansPage.tsx`                                                                                                                                                                                             |
| Page (property)       | `ui/src/features/dashboard/plans/pages/PropertyPlansPage.tsx` — thin wrapper (`paidCheckoutMode="redirect-to-org"`)                                                                                                                                  |
| Routes                | `ui/src/features/dashboard/plans/routes/index.tsx` (`propertyPlansRoute`); org route in `org/routes/index.tsx`                                                                                                                                       |
...
- You can open **Plans & Billing** from a property or from the organization — the plans and prices are the same. When you confirm **Continue to payment**, checkout finishes in the organization billing view.
