...
**Billing is org-level only** (one `org_subscriptions` row covers enrolled properties), but hosts can **browse** Plans & Billing from either the org or property sidebar without leaving their current context. Property `/plans` renders the same tier catalog (`OrgPlansPage` with `paidCheckoutMode="redirect-to-org"`) with **Plans** and **Compare** tabs only. Org `/plans` is the billing hub — **Billing** is the first tab, then Plans / Compare. Choosing a tier still opens `PlanReviewDialog` in place; **Continue to payment** from the property mirror (or feature-gate modal) creates the PayMongo checkout session then opens org **`?tab=billing`** — no second review modal. Downgrades apply immediately from either surface (no payment step).
...
- **Layout:** current-plan summary banner (Free when there is no paid org subscription, otherwise the active tier), then tabs. **Org:** **Billing** (first) / Plans / Compare. **Property:** Plans / Compare only — **Manage subscription** jumps to org Billing.
...
- **Billing tab (org only):** `PlanBillingPanel` — plan, price, renewal date, and `OrgPlanTransactions` (up to 20 `org_payment_transactions` rows with status/method). Default landing tab on org Plans.
...
- **`suspended`:** dashboard restricted to **Plans & Billing** and **Help & Support** only (`RequirePropertySubscriptionAccess` — reads `property-entitlements` `status: suspended`, not a silent Free fallback); guest-facing booking flows unaffected. Server `requirePropertyFeature` / `requireOrgPropertyFeature` reject paid actions while suspended. Team seats pooled across the org's enrolled properties are clawed back to Free's budget on suspension (`reconcileTeamSeatsForProperty`, run per affected property by the billing cron).
...
| E2E                   | `ui/e2e/features/plans/org/orgPlanDowngrade.spec.ts` (mocked downgrade review + guards)                                                                                                                                                              |
