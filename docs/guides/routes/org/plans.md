...
- **Layout:** current-plan summary banner (Free when there is no paid org subscription, otherwise the active tier), then tabs. On phone, banner actions stack full-width with a short **Manage** label (full **Manage subscription** from `sm` up). **Org:** **Billing** (first) / Plans / Compare. **Property:** Plans / Compare only — **Manage subscription** jumps to org Billing.
