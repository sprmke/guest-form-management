...
| Toolbar       | Search, status, type, grid/list                        | Type filter: inside tower / outside tower / motorcycle. `max-lg`: Status + Type fold into an `AdminListRefineSheet` beside the search box (inline `Select` strip is `lg+` only) |
