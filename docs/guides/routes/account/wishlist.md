...
Grid of properties the signed-in guest has "hearted" (saved) while browsing the marketing site. Same save/unsave action as the heart icon on property listing and detail pages — this page is just the guest's saved-properties view.
...
- Q: A guest asks why a property disappeared from their wishlist.
  A: They likely un-hearted it (there's a confirm step before it's removed), or the listing itself may no longer be public. Wishlisting doesn't reserve a stay or hold dates.
...
1. Tap the heart on a `WishlistPropertyCard` → confirm dialog ("remove from wishlist?").
...
- Saving/unsaving from any explore page (listing grid, property detail) is reflected here on next load — there is no separate wishlist-only save action.
...
- [Profile](./profile.md)
- [Properties](../properties.md) — save heart / wishlist gate
