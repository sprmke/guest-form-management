...
**Self-serve property-stay linking (Phase 7):** after sign-in, if the guest has confirmed property stays that signaled `need_parking` and aren't already linked (`useLinkableParkingBookings` → `list-linkable-property-bookings`), the first screen is a **Which stay?** chooser (`ParkingStayChooser`) — one row per stay (property name + compact dates **Sep 24-28, 2026** + unit) and a final option **"Different booking — enter details"**.
