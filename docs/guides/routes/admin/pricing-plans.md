...
- **Edit** opens a **`ResponsiveModal`**: name, tagline, sort order, **list price**, **discount %**, AI monthly credits, boolean feature toggles, team max members, active flag. Live preview shows the host checkout price.
- Saving calls **`pricing-plans`** PATCH; does not retroactively change in-flight PayMongo periods.
...
| PATCH  | `pricing-plans` | Update subscription plan; rejects commission plan ids / `pricingModel: commission`                                                          |
...
Hosts do not use this page. They manage plans per listing on the property **Plans & Billing** page.
