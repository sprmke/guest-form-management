...
6. Admin replacement of guest documents (`upload-booking-asset` with `payment_receipt`, `valid_id`, `pet_vaccination`, or `pet_image`) while the booking is in the same revert-eligible statuses as (5) also sets status back to `PENDING_REVIEW` (same rationale as receipt/ID changes on the public form).
...
- Checkbox panel toggles query params on submit: `saveToDatabase`, `saveImagesToStorage`, `updateGoogleCalendar`, `updateGoogleSheets` (each `true`/`false`). **Filled GAF / pet request PDFs** are not produced on `submit-form`; they run on admin **`PENDING_REVIEW` → `PENDING_GAF` / `PENDING_DOCUMENTS`** via `workflowOrchestrator` (`generatePdf` dev control on `/bookings/:id`). `submit-form` does not accept a `sendEmail` toggle because workflow emails are not sent there. Defaults: in dev/testing panel, **all start unchecked** except when `isProduction && isDevMode` (then they default true—a niche case for prod+`dev=true`).
...
| ------------------------------- | ------ | ------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `submit-form`                   | POST   | anon (public)                  | Multipart form processing, overlap check, change detection, DB/storage/PDF (+ optional calendar/sheets), **no workflow emails**; updates from `/form` while status is revert-eligible (pending-docs stages or `READY_FOR_CHECKIN`) set `PENDING_REVIEW` only when `compareFormData` changed fields are workflow-sensitive (see §5.2)                                                          |
...
| `upload-booking-asset` _(new)_  | POST   | admin JWT                      | Multipart upload; `assetType` → Storage bucket + DB column (includes `guest_balance_payment_receipt` → `sd-refund-receipts` / `guest_balance_payment_receipt_url`); guest doc types `payment_receipt` / `valid_id` / `pet_vaccination` / `pet_image` also revert revert-eligible statuses → `PENDING_REVIEW` (see §5.2)                                                                       |
...
| `sd-refund-cron` _(Phase 4)_    | POST   | cron/internal **or** admin JWT | **Global:** empty/`{}` body — every `READY_FOR_CHECKIN` row past checkout + grace (Manila), settlement complete → `READY_FOR_CHECKOUT`. **Scoped:** JSON `{ "bookingId" }` + admin JWT — only that id. **Email:** SD form email is suppressed when check-out is older than `SD_REFUND_CRON_MAX_CHECKOUT_AGE_DAYS` (default 21; `0` = never suppress); transition + calendar + sheet still run |
| `get-sd-form`                   | GET    | anon (public)                  | Query `?bookingId=` — minimal payload for `/sd-form` when status is `READY_FOR_CHECKOUT`. Includes `primary_guest_name`, `check_in_date`, `check_out_date`, plus `next_stay_voucher_code` / `next_stay_voucher_amount` so the reveal step can replay an already-awarded voucher (no re-roll). After reveal, **`VoucherReveal`** shows guest + stay on the voucher card.                       |
| `claim-sd-voucher`              | POST   | anon (public)                  | Body `{ bookingId }`. Idempotently rolls a code from `_shared/voucher.ts#VOUCHER_WIN_POOL` and persists `next_stay_voucher_code` / `_amount` / `_awarded_at`. Re-calls return the existing voucher with `alreadyAwarded: true`. Status guard: only available while `READY_FOR_CHECKOUT`.                                                                                                      |
| `submit-sd-form`                | POST   | anon (public)                  | Guest submits refund preference (`refund` body); optional `guestFeedback` (stored when sent). → `PENDING_SD_REFUND` via orchestrator                                                                                                                                                                                                                                                          |
| `send-sd-refund-form-email`     | POST   | admin JWT                      | Re-send SD form link; status must be `READY_FOR_CHECKOUT`; updates `sd_refund_form_emailed_at`                                                                                                                                                                                                                                                                                                |

**`/sd-form` cash pickup:** No guest-entered note — the stepper shows fixed policy copy (cash balance/SD paid in cash, staff on Azure premises, contact Facebook/Airbnb before leaving). The legacy **`sd_refund_cash_pickup_note`** column was removed in migration **`20260607120000_drop_sd_refund_cash_pickup_note.sql`**.
...
**Auth**: `verify_jwt = false` for `submit-form`, `get-booked-dates`, `get-form`, `get-sd-form`, `submit-sd-form`, `claim-sd-voucher`. Admin functions (`list-bookings`, `transition-booking`, `upload-booking-asset`, `send-sd-refund-form-email`, `cancel-booking`, `cleanup-test-data`, `google-mail-oauth-start`, `google-mail-oauth-status`, `google-mail-oauth-disconnect`, etc.) have `verify_jwt = false` in `config.toml` (Kong's HS256 gate rejects ES256 tokens) and call `verifyAdminJwt(req)` from `_shared/auth.ts` as the sole security boundary. `google-mail-oauth-callback` is public (Google redirect) and does not use `verifyAdminJwt`; it validates OAuth `state` server-side. Scheduled functions (`gmail-listener`, `sd-refund-cron`) also use `verify_jwt = false`; hosted recurrence is configured with **`pg_cron` + `net.http_post`** (often with secrets in Vault) per [Supabase scheduling guide](https://supabase.com/docs/guides/functions/schedule-functions). Admin “Run … now” buttons call the same HTTP endpoints with a session JWT (see scheduling doc above for operational detail).
...
- `PERMIT_APPROVER_EMAIL` — comma-separated **`From`** addresses allowed to trigger GAF/pet approval handling in `gmail-listener` (the **permit / Azure mailbox that replies** with `APPROVED GAF.pdf`). It is **not** the outbound Resend `From` (`mail@kamehomes.space`), **not** the `To`/`Cc` on the GAF request, and **not** “the inbox we poll.” When this var is non-empty, outbound request emails that appear in the same Gmail history are skipped with `sender_not_allowed`—that is expected. Gmail replies sent as `you@gmail.com` do **not** match `you+tag@gmail.com` in the allow-list unless the visible `From` header actually includes the tag.
- `SD_REFUND_CRON_GRACE_MINUTES` — Grace period in minutes before auto-`READY_FOR_CHECKOUT` after checkout (default: `15`)
- `SD_REFUND_CRON_MAX_CHECKOUT_AGE_DAYS` — For `sd-refund-cron` transitions, if wall-clock time since **check-out** exceeds this many days, the run still moves to `READY_FOR_CHECKOUT` but **does not** send the guest SD form email (avoids mailing long-past stays). Default `21`. Set to `0` to always send when the cron transitions (legacy behavior for email only).
