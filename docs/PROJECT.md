...
**Scheduled jobs (deep dive + testing):** see **`docs/SCHEDULED_JOBS_AND_TESTING.md`** — how `pg_cron` + `pg_net` invoke these URLs on Supabase Cloud, why `schedule` is not in `config.toml` locally, env vars, curl/admin UI test steps, and idempotency. After `supabase db reset`, migration `20260504100000_enable_pg_cron.sql` ensures `pg_cron` exists; you still re-run the doc’s **`cron.schedule`** SQL (job rows are not migrated).
...
- `PERMIT_APPROVER_EMAIL` — sender allow-list for `gmail-listener` approvals; only emails whose `From` address matches this comma-separated list are processed for GAF/Pet approvals
