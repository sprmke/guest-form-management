...
This document describes the **guest-form-management** codebase as implemented in the repository: purpose, architecture, data model, API surface, UI flows, integrations, environment configuration, and known product notes (including items tracked in `TODOS.md` that are not yet built).
...
| `TODOS.md` | Product backlog and completed tasks |
...
|-------|-----------|--------|
| `/` | `GuestFormPage` | Main guest form (new or `?bookingId=` update) |
| `/success` | `GuestFormSuccessPage` | Post-submit summary; requires `?bookingId=` |
| `/calendar` | `CalendarPage` | Date range picker; navigates to `/?checkInDate=&checkOutDate=` |

**Note:** `TODOS.md` mentions a desired change (root = calendar, `/form` = guest form). That is **not** the current routing; calendar lives at `/calendar`.
...
1. User may open `/calendar`, select check-in and check-out (respecting booked ranges and past dates), then **Proceed** → navigates to `/?checkInDate=YYYY-MM-DD&checkOutDate=YYYY-MM-DD`.  
...
1. URL includes **`?bookingId=<uuid>`** (sanitized on client and server if query junk is appended).  
...
- Description includes link to **`https://kamehomes.space?bookingId=...`** with `&testing=true` or `&dev=true` for non-testing mode (see `createEventData`—calendar link query params).  
...
- **`GOOGLE_SPREADSHEET_ID`**: use a dev spreadsheet in local `.env` if desired (`TODOS.md` mentions sprmkedev—**not hardcoded** in repo).
...
## 13. Roadmap / gaps (from `TODOS.md` vs code)

Examples **not** fully reflected in code or only partially done:

- Root route = calendar and `/form` for the form.  
...
- Calendar event still appends `dev=true` in some branches when not testing—`TODOS.md` flags removing `dev=true` from links.

Use **`TODOS.md`** as the live product backlog; this doc describes **current** implementation unless a section explicitly calls out planned work.
...
*Last updated from repository analysis (internal documentation).*
