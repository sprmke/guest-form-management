...
| Local plan seed     | Entire local DB on **Free** (`kame-home` + `kame-homes` enrolled Free)                                                                     |
...
**Local QA gap:** Paid-tier **live** checks need a temporary local plan change; this pass verified Free live + code/matrix for higher tiers. Re-run skill § Plan matrix after bumping `org_subscriptions.plan_id`.
...
5. Local `kame-home` org temporarily set to **Pro (`growth`)** for paid-gate smoke (was Free).
