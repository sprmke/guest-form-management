...
- **Editor UI click-test.** `StayGuidePageEditor` + panel type-check and mirror the verified
  `PropertyShowcasePageEditor`, but weren't driven in a signed-in dashboard session (needs org
  auth). Exercise: template picker → live preview switch, section reorder/hide, style controls,
  chapter rich-text autosave to `property-templates-settings`, leave-confirm.
- **Rich-text ink tone.** `StayGuideRichContent` hard-codes body `#404040` / `#D4D4D4`; on
  some template palettes (e.g. Haven sand) it reads faint. Should pull from the
  `.showcase-scope` ink vars like the showcase section body does.
- **Template picker thumbnails.** Stay Guide uses a labelled 6-card radio grid, not Showcase's
  live scaled-preview thumbs (`ShowcaseTemplatePreviewThumb` is `ResolvedPropertyDetail`-shaped).
  Parity item if wanted.
- **`getting-in` single-block chapters** render fine but with a redundant card frame around one
  block — consider unwrapping when `blocks.length === 1`.
- **Gating model divergence** — see progress log. The concurrent workstream made Public Pages
  explore-open (save-gated), not route-gated. If the product wants a hard Pro access block on
  the editor (the original ask), re-add `<RequirePropertyFeature feature="customPages">` in
  `custom-pages/routes/index.tsx` **and** re-flip `customPages` to Pro (undo `20261212120000`)
  — coordinate with that workstream first.
