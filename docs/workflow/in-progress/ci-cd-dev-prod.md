...
stage: in-progress
kind: plan
tags: [workflow, planned, deployment, ci-cd, supabase, vercel, multi-tenancy]
updated: 2026-08-08
---

# CI/CD Dual-Track Implementation Plan

> **For agentic workers:** Implement task-by-task. Use checkbox (`- [ ]`) tracking. **Do not** mutate legacy Supabase or Vercel Production env vars unless a task explicitly targets cutover (Phase D) and the user unlocks with **`kamewave`** where shell rules require it.

**Design:** [`../intake/ci-cd-dev-prod-design.md`](../intake/ci-cd-dev-prod-design.md)
...
- [ ] Quick verify (table above) passed
- [ ] (Optional) Auth on `kame-homes` still works — `/for-hosts/login`
...
- [ ] Quick verify passed **or** full deploy below completed
- [ ] Edge secrets minimum set on **`fwor…`** (see Part E)
- [ ] Smoke on `kame-homes` (Part F)
...
- [ ] Add `--ci` / `CI=1` + `DEPLOY_CONFIRM=dev|prod` to deploy/backup paths for non-interactive CI.
- [ ] Reject `--skip-backup` when `CI=1`.
- [ ] Assert linked/target ref equals `SUPABASE_PROJECT_REF`.
- [ ] Assert target ref **≠** `LEGACY_PROD_PROJECT_REF` (env) when multi-tenant tree deploy (`--allow-multi-tenancy` path).
- [ ] Thin wrappers: `scripts/deploy/ci-deploy.sh`, `ci-smoke.sh`.
- [ ] `bash -n` + dry-run where possible.
...
- [ ] Create Environment `development`: `SUPABASE_ACCESS_TOKEN`, `SUPABASE_PROJECT_REF` = `fworvijbrwpyngycotbz`, `SUPABASE_DB_URL`, `LEGACY_PROD_PROJECT_REF=zfttdwtceyqszyeyhilc` for deny-list.
- [ ] Create `preproduction` + required reviewers when preprod project exists.
- [ ] Create `production` **empty** or without real mt-prod secrets until cutover; required reviewers; deployment branch `main` only later.
- [ ] Repo secrets never hold LEGACY service role for auto workflows.
...
- [ ] `cd-dev.yml`: `on.push.branches: [develop]` → quality steps → `ci-deploy.sh dev` → smoke → concurrency queue `supabase-mt-dev`.
- [ ] `cd-preprod.yml`: `workflow_dispatch` → Environment `preproduction`.
- [ ] `cd-prod.yml`: `workflow_dispatch` with hard `if: secrets` present OR `CUTOVER_ENABLED` — otherwise fail with message “cutover not enabled”.
- [ ] `cd-rollback.yml`: dispatch; env input; production Environment gate.
- [ ] Ensure `ci.yml` runs on PRs targeting `develop` and `main`.
...
- [ ] `main`: require quality; prefer PR-only; prevent force-push.
- [ ] `develop`: require quality; prefer PR-only.
- [ ] Optional: CODEOWNERS for `supabase/migrations/` and deploy scripts.
...
- [ ] Update `docs/architecture/deployment.md` matrix (dual-track).
- [ ] Update `docs/archive/operations/dev-staging-environment.md`: `develop` + Staging + mt-dev.
- [ ] Update `docs/archive/operations/production-deployment.md`: dual-track + cutover pointer; reaffirm multi-tenant ban on LEGACY.
- [ ] Agent rules: dual-track note in `no-prod-deploy` / project-context (see Phase E — do early if not already).
- [ ] `scripts/README.md`: CD workflow names.

**Verify:** New agent session reading rules cannot conclude “deploy multi-tenant to main’s Supabase”.

---

## Phase D — Cutover (separate approval; later)
...
| Doc                                 | Content                                                                               |
| ----------------------------------- | ------------------------------------------------------------------------------------- |
| Design                              | [`docs/workflow/intake/ci-cd-dev-prod-design.md`](../intake/ci-cd-dev-prod-design.md) |
...
- Supabase product “database branching” feature (we use separate projects)
