...
status: approved
tags: [search, marketing, performance, pagination]
updated: 2026-08-05
---
...
**Implementation status (2026-08-05):** Phase 1A page-only enrichment and Phase 1B paged lean-candidate loading are complete for `list-public-*`. The interim hybrid reads deterministic 1,000-row ranges and fails closed above 20,000 candidates; `search-listings` cap parity remains Phase 1C.
...
New or extended endpoint(s), e.g. place-index with `offset`/`limit` on **groups**, each group carrying preview items + `totalInPlace`. Exact function names left to the implementation plan.
