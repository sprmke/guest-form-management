...
**Implementation plan (v1 slice):** [`booking-workflow-configurable-docs.md`](../in-progress/booking-workflow-configurable-docs.md) — **in progress**; tasks 1–7 below are implemented pending human verify. Locked decisions: backfill completions JSONB; empty requirements → **`READY_FOR_CHECKIN`** (D2 — skip `PENDING_DOCUMENTS`); keep client/server workflow mirror for v1; Impeccable UI on settings + WorkflowPanel.
...
| #   | Task                                                                                                                                                                                                                                                                                            | Area                 | Notes                                                                                                                                                              |
| --- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| 1   | Design & migrate the generic `DocumentRequirement` model (`documentRequirements.ts`, `developments.settings.workflowDefaults`, `app_settings.document_requirements_override`)                                                                                                                   | schema               | **Shipped (v1)** — [`../in-progress/booking-workflow-configurable-docs.md`](../in-progress/booking-workflow-configurable-docs.md) Tasks 1–2; pending human verify. |
| 2   | Add `document_requirement_completions` JSONB to `guest_submissions`; decide migration path for existing named columns (`gaf_completed_at`, etc.)                                                                                                                                                | schema               | **Decided: backfill** into JSONB; dual-read during cutover. **Shipped (v1)** Task 1; pending human verify.                                                         |
| 3   | Generalize `statusMachine.ts` nested-completion + calendar-prefix logic to iterate `documentRequirements[]` instead of GAF/parking/pet `if` branches                                                                                                                                            | orchestrator         | **Shipped (v1)** Task 3; client mirror kept in lockstep (audit task 10 deferred). Pending human verify.                                                            |
| 4   | Generalize `workflowOrchestrator.ts`'s `PENDING_REVIEW → PENDING_DOCUMENTS` transition to loop over document requirements for PDF generation + email sends, gated by `approvalSource` and the existing `propertyAutomationToggles`; **D2:** empty list → `READY_FOR_CHECKIN`                    | orchestrator         | **Shipped (v1)** Task 4; pending human verify.                                                                                                                     |
| 5   | Add `sync_calendar` / `sync_sheets` property-level toggles and check them in the orchestrator before calling `CalendarService`/`SheetsService`                                                                                                                                                  | schema, orchestrator | **Shipped (v1)** Tasks 1 + 4; pending human verify.                                                                                                                |
| 6   | Add an admin settings UI for per-property document requirements, and Calendar/Sheets sync toggles                                                                                                                                                                                               | UI                   | **Shipped (v1)** Tasks 5–6 (Impeccable); pending human verify.                                                                                                     |
| 7   | Update `WorkflowPanel`/`PendingDocumentsSubTree`, `workflowDevControls.ts`, and the workflow-sensitive-field revert list to be data-driven from `documentRequirements[]`                                                                                                                        | UI                   | **Shipped (v1)** Task 7 (Impeccable); pending human verify.                                                                                                        |
