...
| [`pwa-installable-offline-push.md`](./pwa-installable-offline-push.md)                       | not started  | Installable PWA for the host/admin dashboard — manifest + service worker, cross-platform Web Push wired to the Notification Center, offline read (allowlisted) + offline write/sync (status transitions + inbox replies). 6 phases. |
