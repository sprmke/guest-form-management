...
- The `GmailReconnectProvider` precedent for mounting a single always-on provider once in `AdminLayout.tsx`, running across every admin page for a tenant.
...
1. **v1 event catalog**: new booking submitted (`PENDING_REVIEW`), ready for check-in, ready for check-out, SD refund due, Gmail auto-approval of GAF doc, Gmail auto-approval of pet doc, and new inbound guest chat/inbox message. Modeled as an extensible `TEXT + CHECK` type union (matching this repo's booking-status convention) so more event types (guest portal submissions, team/org membership changes, maintenance/finance reminders) can be added later without a redesign — explicitly **not built in v1**.
...
- **`gmail-listener/index.ts`**: right after the live-approval `WorkflowOrchestrator.transition(...)` call, add `createNotification` for `booking_gaf_auto_approved` / `booking_pet_auto_approved`. Only from the **live listener path** — explicitly skip `gmail-backfill-approvals` (would spam notifications for old resolved bookings) and `reconcileManualIncompleteApprovals` (re-applying an existing approval, not new information).
...
- Wrap `<NotificationsProvider>` inside the existing `<GmailReconnectProvider>` — one instance for the whole admin tree.
