...
This plan is a continuation of `docs/planning/planned_modules/2026-07-29-smart-search-bar.md` and should not start implementation until that plan has shipped. It reuses, rather than duplicates, the shared infrastructure that plan introduces:
...
4. **Dependency**: this plan does not start until the smart-search-bar plan has shipped, so it can build on that plan's shared infra instead of duplicating date-conflict logic or the `properties.city` column.
...
Sort: `recommended` (default — rating desc, ties by review count), `price-low`/`price-high` (`app_settings.weekday_nightly_rate`), `rating` (`guest_reviews` avg, batched), `reviews` (`guest_reviews` count, batched), `newest` (`properties.created_at DESC` — real, replacing the fake `isNew` sort).
...
Sort: `recommended` (default, `created_at DESC` as the neutral default), `price-low`/`price-high` (`settings.priceRangeMin/Max`), `newest` (`developments.created_at DESC`, real). **`rating` is dropped** — no development-level reviews exist; shipping it as a no-op sort would ship a filter option with no real effect.
...
Sort: `price_asc` (default), `price_desc`, `tower` — unchanged from `parkingSlotFilters.ts`, already fully real; parking never had a `rating`/`bedrooms`-shaped option to begin with, so it needed the least correction.
