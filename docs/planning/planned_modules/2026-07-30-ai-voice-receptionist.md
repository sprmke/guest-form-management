...
- **No existing rate-limit/quota/usage-tracking pattern anywhere in the codebase** (confirmed by grep) — the still-unbuilt `2026-07-29-smart-search-bar.md` plan calls per-IP throttling "first of its kind." A session-cap/cost-tracking mechanism for this feature will be new infrastructure, not a reuse.
