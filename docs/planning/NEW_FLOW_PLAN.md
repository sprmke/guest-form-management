...
- Historically, dev behavior used **query params** (`?dev=true`, formerly `?testing=true`) plus checkboxes for optional submit side effects. **Current app:** `?testing=true` and test bookings are **removed** — use **local/staging Supabase** for safe trials; public `/form` may still show save/storage/calendar/sheet toggles when **`!production`** or **`?dev=true`**.
...
- **Guest `/form`:** optional dev API toggles when non-production or `?dev=true` (no workflow emails/PDFs from submit). **Admin `/bookings/:id`:** full workflow dev-controls when signed in as allow-listed admin (`RequireAdmin`). **No** test-booking column, **no** `?testing=true`, **no** `[TEST]` prefixes — staging/local env replaces inline test mode.
...
ui/src/features/guest/form/components/GuestForm.tsx
  - Optional dev API toggles when non-production or `?dev=true` (no `testing` query param).
...
| **Q3.2**             | **Dev controls:** the **workflow dev-controls block** (checkboxes including **Generate GAF / pet request PDFs**) is **admin-only** on `/bookings/:id`. Public guest `/form` may still expose **save / storage / calendar / sheet** toggles when non-production or `?dev=true` — **not** PDF or workflow emails. Authenticated allow-listed admin enables the full admin dev panel on `/bookings/:id`.                                                                                                                                                                                                                                                                                                                                                                                                                                     |
...
- No `?testing=true` or test-booking pipeline; optional `?dev=true` on `/form` is only for non-prod or prod debugging of save/storage/calendar/sheet flags.
