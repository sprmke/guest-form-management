...
- **Phase 3.1 — Bookings UX upgrade (post-Phase 3).** ✅
  ...
  - **Backward overrides** were added to `MANUAL_OVERRIDE_GRAPH` on **both** server (`statusMachine.ts`) and client mirror (`workflow.ts`): `PENDING_GAF → PENDING_REVIEW`, `PENDING_PARKING_REQUEST → PENDING_GAF`, `PENDING_PET_REQUEST → {PENDING_PARKING_REQUEST | PENDING_GAF}`, `READY_FOR_CHECKIN → {PENDING_DOCUMENTS | PENDING_PET_REQUEST | PENDING_PARKING_REQUEST | PENDING_GAF | PENDING_REVIEW | PENDING_SD_REFUND}`, `PENDING_SD_REFUND_DETAILS → READY_FOR_CHECKIN`, `PENDING_SD_REFUND → PENDING_SD_REFUND_DETAILS`. `COMPLETED` and `CANCELLED` stay terminal.
