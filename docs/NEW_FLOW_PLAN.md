...
> Status: **§6.1 decisions locked**; **§6.2** = **Q7.4** (surprise setup) refinement only. **Phases 0–2 are implemented in-repo** (see `docs/TODOS.md`). **Phase 3+** still planned.
...
5. **Phase 3 — Transitions (no email listener yet).** Manual `transition-booking` endpoint + UI forms. Guest acknowledgement / ready-for-check-in emails working.
6. **Phase 4 — Email listener + cron.** `gmail-listener` + `sd-refund-cron` wired, idempotent.
...
- **Phase 3:** `transition-booking` + `workflowOrchestrator` + `/bookings/:bookingId` workflow UI + server `verifyAdminJwt` for admin functions + acknowledgement / ready-for-check-in emails per matrix.
