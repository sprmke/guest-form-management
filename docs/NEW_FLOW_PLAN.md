...
> Status: **§6.1 decisions locked**; **§6.2** = **Q7.4** (surprise setup) refinement only. **Phases 0–4 are implemented in-repo** (see `docs/TODOS.md`). **Phase 5+** still planned.
...
supabase/functions/submit-form/index.ts
  ...
  - Always DB + storage (overridable via dev flags, see Q3.2). **No guest-submit PDF** — see Q3.1.
...
- **Phase 5:** Remove side effects from `submit-form`; retire `?dev=true` / `?testing=true`; add Test Submit button to guest form.
