...
| ------------------------------------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `submit-form` **(changed)**                 | POST   | Saves DB + storage + PDF and sets initial status `PENDING_REVIEW`. **Workflow emails are not sent here**; booking acknowledgement, GAF request, pet request, and parking broadcast are sent only on `PENDING_REVIEW → PENDING_GAF`. |
...
7. **Phase 5 — Submit-form cleanup.** In progress: workflow email side effects removed from `submit-form` (now only sent during `PENDING_REVIEW → PENDING_GAF` transition). Remaining: retire query-param dev flags and add Test Submit button.
