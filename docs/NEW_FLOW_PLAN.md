...
1. ✅ **Phase 0 — Backup & schema.** Snapshot **`guest_submissions_backup_20260501`** (`20260501000000_*`); nullable workflow columns + approved/request PDF URLs + `processed_emails` + `gmail_listener_state` + four storage buckets (`20260501000002`–`20260501000010`). **`20260428120000_*`** (`PENDING_DOCUMENTS`) runs **before** that batch in filename order. No app-code requirement by itself — see **`docs/MIGRATION_RUNBOOK.md` §1**.
...
3. **Phase 2 — State machine read path.** ✅ Migration `20260502000000_widen_status_enum.sql` backfills legacy rows, widens `status` to `TEXT + CHECK`, sets `DEFAULT 'PENDING_REVIEW'`. `_shared/statusMachine.ts` (server) + `ui/src/features/admin/lib/workflow.ts` (client) created with full transition graph, calendar meta map, and sub-form requirements. `get-booked-dates` + `databaseService.checkOverlappingBookings` updated to block on any non-`CANCELLED` status. `StatusBadge` already rendering in the bookings list (wired in Phase 1).
