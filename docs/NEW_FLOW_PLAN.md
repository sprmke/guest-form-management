...
- **Phase 3.1 — Bookings UX upgrade (post-Phase 3).** ✅
  ...
  - **Backward overrides** were added to `MANUAL_OVERRIDE_GRAPH` on **both** server (`statusMachine.ts`) and client mirror (`workflow.ts`): `PENDING_GAF → PENDING_REVIEW`, `PENDING_PARKING_REQUEST → PENDING_GAF`, `PENDING_PET_REQUEST → {PENDING_PARKING_REQUEST | PENDING_GAF}`, `READY_FOR_CHECKIN → {PENDING_PET_REQUEST | PENDING_PARKING_REQUEST | PENDING_GAF | PENDING_REVIEW}`, `PENDING_SD_REFUND → READY_FOR_CHECKIN`. `COMPLETED` and `CANCELLED` stay terminal.
  - **Email re-fire safety:** `workflowOrchestrator` now gates the ready-for-check-in email on `fromStatus ∈ {PENDING_GAF, PENDING_PARKING_REQUEST, PENDING_PET_REQUEST}`, and the `Send Ready-for-Check-in Email` dev control's `isRelevant` was tightened to those same three statuses. A backward `PENDING_SD_REFUND → READY_FOR_CHECKIN` therefore never re-emails the guest, even if the dev control gets toggled. See `.cursor/rules/booking-workflow.mdc` §2.2 for the full canonical list of override edges.
  - **Calendar title detail for `PENDING_DOCUMENTS`:** event prefix now resolves to the exact currently pending nested stage (`PENDING GAF`, `PENDING PARKING REQUEST`, or `PENDING PET REQUEST`) from completion markers (`*_completed_at` / approved PDFs / parking endorsement URL). Color remains the same pending-docs yellow family (`colorId: 5`).
