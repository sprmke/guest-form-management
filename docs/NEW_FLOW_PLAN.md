...
| `balance`                     | `NUMERIC`     | Stored as **`booking_rate - down_payment`** (same as pre–Total-Guest-Balance column). **Display “Total Guest Balance”** in the review step sums **`balance + security_deposit + pet_fee + parking_rate_guest + guest_additional_fee`** (see **Q2.1** in §6).                                                                                                                                     |
...
| `guest_additional_fee`        | `NUMERIC`     | Optional misc. charges (early CI, late CO, decor, etc.). Set from **Pending Review** pricing form; included in total-due math and ready-for-check-in email when amount is greater than zero. Synced to Sheets column **AX**.                                                                                                                                                       |
...
| ------------------------------------------- | ------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `submit-form` **(changed)**                 | POST   | Saves DB + storage + PDF and sets initial status `PENDING_REVIEW`. **Workflow emails are not sent here**; booking acknowledgement, GAF request, pet request, and parking broadcast are sent only on `PENDING_REVIEW → PENDING_DOCUMENTS`. |
...
| **Q2.1**             | **Total Guest Balance** (admin pricing panel + RFCI email footer) = `booking_rate - down_payment + security_deposit + pet_fee + parking_rate_guest + guest_additional_fee`. DB column **`balance`** remains `booking_rate - down_payment` only. The admin pricing panel shows the full sum as **"Total Guest Balance (paid on or before check-in)"**. **`guest_additional_fee`** defaults to 0; optional line item for early CI, late CO, decor, etc.                                                                                                                                                                                                                                                                                                                                                                                             |
