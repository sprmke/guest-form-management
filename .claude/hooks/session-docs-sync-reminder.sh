...
MSG='DOCS SYNC (mandatory): Material code/behavior changes must update matching docs in the SAME change — not later. Route/page UX → docs/guides/routes/* (route-guides skill). API/env/architecture → docs/PROJECT.md. Plans/tiers → docs/architecture/plans-feature-matrix.md + Plans guides. Booking/auth invariants → .cursor/rules/booking-workflow.mdc or admin-auth.mdc. Invoke documentation-maintenance skill before claiming done. Canonical: CLAUDE.md § Docs are the source of truth · .cursor/rules/documentation-maintenance.mdc.'
