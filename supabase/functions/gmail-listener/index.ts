 ...
 *
 * Secrets required:
 *   GMAIL_API_WEB_CLIENT_JSON, GMAIL_OAUTH_TOKEN_ENCRYPTION_KEY,
 *   per-property refresh token in `gmail_mail_integration` (Connect Gmail).
 *   SUPABASE_URL               — set automatically by Supabase
 *   SUPABASE_SERVICE_ROLE_KEY  — set automatically by Supabase
 *
 * Reference: docs/planning/NEW_FLOW_PLAN.md §3.4, .cursor/rules/booking-workflow.mdc §3
...
async function loadHistoryId(propertyId: string): Promise<string | null> {
...
async function resolveListenerMailboxEmail(propertyId: string): Promise<string> {
  const { data } = await supabaseAdmin()
...
async function saveHistoryId(propertyId: string, historyId: string): Promise<void> {
  const sb = supabaseAdmin();
  const email_address = await resolveListenerMailboxEmail(propertyId);
  ...
  if (error) console.error(`[gmail-listener] Failed to save historyId for ${propertyId}:`, error);
}
...
async function processMessage(
  ...
  console.log(
    `[gmail-listener] Message ${messageId.slice(0, 16)}… Subject: ${subject.slice(0, 80)}`
  );
  ...
  if (allowedApprovers.length > 0 && !allowedApprovers.includes(senderEmail)) {
    ...
    return {
      action: 'skipped',
      reason: `sender_not_allowed:${senderEmail || 'unknown'}`,
      kind: parsed.kind,
    };
  }
  ...
  console.log(
    `[gmail-listener] Found approval (${parsed.kind}): ${parsed.checkInDate} → ${parsed.checkOutDate}`
  );
  ...
  try {
    ...
    if (e?.ambiguous) {
      ...
      return {
        action: 'skipped',
        reason: `ambiguous_multiple_bookings (${e.matchCount})`,
        kind: parsed.kind,
      };
    }
    throw e;
  ...
  if (!booking) {
    ...
    return {
      action: 'skipped',
      reason: `no_booking_found_for_${expectedStatuses.join('_or_')}`,
      kind: parsed.kind,
    };
  }
  ...
  console.log(
    `[gmail-listener] Marking document sub-status complete for booking ${bookingId}: ${payload.document_completion_target}`
  );
 ...
 */
async function reconcileManualIncompleteApprovals(
  propertyId: string
  ...
  if (gafErr) {
    ...
    for (const row of gafRows ?? []) {
      ...
      try {
        ...
        console.error(`[gmail-listener] Reconcile GAF failed for ${row.id}:`, (e as Error).message);
      }
    }
  }
  ...
  if (petErr) {
    ...
    for (const row of petRows ?? []) {
      ...
      try {
        ...
        console.error(`[gmail-listener] Reconcile pet failed for ${row.id}:`, (e as Error).message);
      }
    }
  }
...
async function listPropertyIdsToPoll(): Promise<string[]> {
  const sb = supabaseAdmin();
  ...
  if (error) {
    console.error('[gmail-listener] Failed to list Gmail integrations:', error.message);
  }

  const ids = [
    ...new Set(
      (data ?? [])
        .map((row) => (row.property_id as string | null)?.trim())
...
type PropertyPollResult = Record<string, unknown>;

async function pollPropertyMailbox(propertyId: string): Promise<PropertyPollResult> {
  let accessToken: string;
  try {
    ({ accessToken } = await getGmailAccessTokenUnified(propertyId));
  } catch (e: any) {
    console.error(`[gmail-listener] OAuth failed for property ${propertyId}:`, e.message);
    const needsReAuth =
      e?.needsReAuth === true ||
      e.message?.includes('invalid_grant') ||
      e.message?.includes('Reconnect Gmail');
    return { propertyId, success: false, error: e.message, needsReAuth };
  }

  console.log(`[gmail-listener] Gmail OAuth OK for property ${propertyId}`);

  let startHistoryId = await loadHistoryId(propertyId);

  if (!startHistoryId) {
    startHistoryId = await getMailboxHistoryId(accessToken);
    await saveHistoryId(propertyId, startHistoryId);
    console.log(
      `[gmail-listener] Initialized historyId=${startHistoryId} for property ${propertyId} (no backlog)`
    );
    return { propertyId, success: true, initialized: true, historyId: startHistoryId };
  }
  ...
  try {
    ...
    if (e?.historyExpired) {
      ...
      await saveHistoryId(propertyId, newId);
      console.warn(
        `[gmail-listener] History expired for property ${propertyId}. Reset historyId to ${newId}.`
      );
      return { propertyId, success: true, historyReset: true, newHistoryId: newId };
    }
    throw e;
  }
  ...
  console.log(
    `[gmail-listener] property=${propertyId}: ${addedMessageIds.length} new message(s) since historyId=${startHistoryId}`
  );

  if (addedMessageIds.length === 0) {
    await saveHistoryId(propertyId, newHistoryId);
    const reconciled = await reconcileManualIncompleteApprovals(propertyId);
  ...
  for (const messageId of addedMessageIds) {
    ...
    let kind: ApprovalKind = 'gaf';

    try {
      const result = await processMessage(messageId, accessToken, allowedApprovers, propertyId);
...
function aggregatePropertyPollResults(results: PropertyPollResult[]): Record<string, unknown> {
  const successful = results.filter((r) => r.success === true);
  const failed = results.filter((r) => r.success === false);
  const needsReAuth = failed.some((r) => r.needsReAuth === true);
  ...
  return {
    ...
    initialized: sole?.initialized === true || undefined,
    historyReset: sole?.historyReset === true || undefined,
    historyId: sole?.historyId,
    applied: successful.reduce((sum, r) => sum + Number(r.applied ?? 0), 0),
    skipped: successful.reduce((sum, r) => sum + Number(r.skipped ?? 0), 0),
    failed: successful.reduce((sum, r) => sum + Number(r.failed ?? 0), 0) + failed.length,
    reconciled: successful.reduce((sum, r) => sum + Number(r.reconciled ?? 0), 0),
    reconciledGaf: successful.reduce((sum, r) => sum + Number(r.reconciledGaf ?? 0), 0),
    reconciledPet: successful.reduce((sum, r) => sum + Number(r.reconciledPet ?? 0), 0),
    properties: results,
  };
}
...
serve(async (req) => {
  ...
  try {
    ...
    if (hasBearer) {
      ...
      propertyIds = await listPropertyIdsToPoll();
    }
    ...
    if (propertyIds.length === 0) {
      return new Response(
        JSON.stringify({
          ...
          skipped: 0,
          failed: 0,
          message: 'No Gmail integrations configured',
        }),
        { status: 200, headers: { ...corsHeaders(req), 'Content-Type': 'application/json' } }
      );
    }

    const propertyResults: PropertyPollResult[] = [];
    for (const propertyId of propertyIds) {
      propertyResults.push(await pollPropertyMailbox(propertyId));
    }

    const summary = aggregatePropertyPollResults(propertyResults);

    console.log('[gmail-listener] Run complete:', JSON.stringify(summary));
