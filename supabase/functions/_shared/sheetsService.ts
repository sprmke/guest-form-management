...
import { resolveGoogleSheetsAuth } from './propertyGoogleApiAuth.ts';
...
export class SheetsService {
  static async appendToSheet(
    ...
    try {
      ...
      if (existingRow) {
        ...
        const values = this.formatDataForSheet(
          formData,
          validIdUrl,
          paymentReceiptUrl,
          petVaccinationUrl,
  ...
  private static async findRowByBookingId(
    credentials: any,
    bookingId: string
    ...
    try {
      ...
      for (let i = 1; i < values.length; i++) {
        ...
        if (values[i] && values[i][0] === bookingId) {
          // Check the bookingId column (now first column)
          return {
  ...
  private static formatDataForSheet(
    formData: GuestFormData,
    validIdUrl: string,
    paymentReceiptUrl: string,
    petVaccinationUrl: string,
  ...
  static async updateSheetWorkflowStatus(
    ...
    try {
      ...
      const propertyId = propertyIdFromBooking(booking);
      const credentials = await this.getCredentials(propertyId);
      if (!credentials) {
      ...
      if (!existingRow) {
        if (!booking) {
          console.log(
            `No sheet row found for booking ${bookingId} (no booking data to create one)`
          );
          return { success: true, skipped: true };
        }
  ...
  private static async getCredentials(propertyId?: string) {
    return resolveGoogleSheetsAuth(propertyId);
  }
}
