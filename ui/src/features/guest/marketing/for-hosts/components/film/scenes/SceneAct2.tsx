        ...
        <div className="mb-3.5 grid grid-cols-4 gap-3">
          {summary.map(([label, count, tone], index) => (
            ...
            >
              ...
              <p className="mt-1 text-[20px] font-black text-slate-900">{count}</p>
              <span
                className={cn(
        ...
        <div className="grid grid-cols-[1.5fr_0.85fr] gap-4">
          ...
          >
            <div className="flex items-center gap-3 border-b border-slate-100 pb-3.5">
              <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-teal-50 font-black text-teal-700">
                KS
              </div>
              <div>
            ...
            {[
              ...
              >
                ...
                <div className="min-w-0">
                  <p className="text-[11px] font-extrabold text-slate-800">{event.label}</p>
                  <p className="mt-0.5 text-[9px] text-slate-400">{event.sub}</p>
                </div>
      ...
      <PushIn>
        ...
        <div className="relative grid grid-cols-4 gap-2.5" style={reveal(frame, 12)}>
          {BOARD_COLUMNS.map((column, columnIndex) => (
            <div
              key={column.title}
              className="min-h-[236px] rounded-2xl border border-slate-200 bg-slate-50 p-2.5"
            >
              ...
              <div className="space-y-2">
                {column.cards.map((card) => (
                  ...
                  >
                    <p className="text-[11px] font-bold text-slate-700">{card}</p>
                    <p className="mt-0.5 text-[9px] text-slate-400">Aug · 3 nights</p>
                  </div>
                ))}
                {columnIndex === 2 ? (
                  dropped ? (
                    <div
                      className="rounded-xl border-2 border-teal-400 bg-teal-50 p-2.5"
                      style={reveal(frame, 102, 8)}
                    >
                      <p className="text-[11px] font-bold text-teal-700">Rae Reyes</p>
                      <p className="mt-0.5 text-[9px] text-teal-600">Moved to Confirmed</p>
                    </div>
                  ) : (
                    <div className="rounded-xl border-2 border-dashed border-teal-300 bg-teal-50/40 p-2.5 opacity-70">
                      <p className="text-[10px] font-semibold text-teal-500">Drop here</p>
                    </div>
                  )
                ) : null}
              </div>
          ...
          {!dropped ? (
            <div
              className="absolute w-[22%] rounded-xl border-2 border-teal-400 bg-white p-2.5 shadow-xl"
              style={{
                left: `${cardLeftPct}%`,
                top: cardTop,
            ...
            >
              <p className="text-[11px] font-bold text-teal-700">Rae Reyes</p>
              <p className="mt-0.5 text-[9px] text-slate-400">Documents received</p>
            </div>
          ) : null}
        </div>
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <div>
              <p className="text-[11px] font-black text-slate-700">New booking</p>
              <p className="text-[9px] text-slate-400">Same guest form, right in the dashboard</p>
            </div>
          </div>
          <div className="mt-3 grid grid-cols-4 gap-2">
            {['Guest name', 'Stay dates', 'Guests', 'Downpayment receipt'].map((label, index) => (
              <div
                key={label}
                className="flex h-8 items-center rounded-lg border border-slate-200 px-2.5 text-[9px] text-slate-500"
      ...
      <PushIn>
        ...
        >
          <div className="mb-3 flex items-center gap-2">
            ...
            <p className="text-[12px] font-black text-slate-800">AI column matching</p>
              ...
              return (
                ...
                >
                  ...
                  <div className="relative flex w-[120px] items-center">
                    <span className="h-0.5 w-full rounded-full bg-slate-200" />
                    <span
                      className="absolute left-0 h-0.5 rounded-full bg-teal-500"
                      style={{ width: `${t * 100}%` }}
                    />
                    <ArrowRight
                      className="absolute h-3.5 w-3.5 text-teal-500"
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    {done ? (
                      <span className="text-[9px] font-bold text-emerald-600">{confidence}</span>
                    ) : null}
                  </div>
                </div>
              );
        ...
        <div className="mt-4 grid grid-cols-[1.5fr_1fr] gap-4">
          <div
            className="flex items-center gap-1.5 rounded-2xl border border-slate-200 bg-white p-3 shadow-sm"
    ...
    <FilmShell activeLabel="Pricing">
      <PushIn>
        ...
        <div className="grid grid-cols-[0.9fr_auto_1.1fr] items-stretch gap-4">
          ...
          >
            <div className="mb-3 flex items-center gap-2">
              <span className="flex h-8 w-8 items-center justify-center rounded-lg bg-rose-50 text-[12px] font-black text-rose-500">
                A
              </span>
              <p className="text-[12px] font-black text-slate-800">Airbnb calendar</p>
            </div>
          ...
          <div className="flex w-[150px] flex-col items-center justify-center gap-3">
            <div
              className="flex items-center gap-1.5 rounded-full border border-rose-200 bg-white px-2.5 py-1 text-[9px] font-bold text-rose-500 shadow-sm"
            ...
            <p className="text-center text-[8px] font-bold uppercase tracking-wide text-slate-400">
              Two-way · every 15 min
            </p>
            <div
              className="flex items-center gap-1.5 rounded-full border border-teal-200 bg-white px-2.5 py-1 text-[9px] font-bold text-teal-600 shadow-sm"
          ...
          >
            <div className="mb-3 flex items-center gap-2">
              ...
              <p className="text-[12px] font-black text-slate-800">Monaco 2604 · Pricing</p>
            </div>
            <div className="grid grid-cols-4 gap-1.5">
              {Array.from({ length: 12 }, (_, i) => {
                  ...
                  <div
                    ...
                    className={cn(
                      ...
                      arrived
                        ...
                        : blocked
                          ? 'border-slate-200 bg-[repeating-linear-gradient(45deg,#f1f5f9,#f1f5f9_3px,#e2e8f0_3px,#e2e8f0_6px)] text-slate-400'
            ...
            {inbound > 0.7 ? (
              <div
                className="mt-3 flex items-center gap-2 rounded-lg bg-amber-50 px-2.5 py-1.5 text-[9px] font-bold text-amber-700"
