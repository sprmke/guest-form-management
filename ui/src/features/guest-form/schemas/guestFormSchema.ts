  ...
  return z.object({
    ...
    if (data.needParking) {
      if (!data.carPlateNumber) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Car plate number is required when parking is needed", path: ["carPlateNumber"] });
      }
      if (!data.carBrandModel) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Car brand and model is required when parking is needed", path: ["carBrandModel"] });
      }
      if (!data.carColor) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Car color is required when parking is needed", path: ["carColor"] });
      }
      ...
      if (!parkIn?.trim()) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Please select parking check-in date", path: ["parkingCheckInDate"] });
      }
      if (!parkOut?.trim()) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Please select parking check-out date", path: ["parkingCheckOutDate"] });
      }
      if (parkIn && parkOut && parkOut <= parkIn) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Parking check-out must be after check-in", path: ["parkingCheckOutDate"] });
      }
    }

    if (data.hasPets) {
      if (!data.petName) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Pet name is required when bringing pets", path: ["petName"] });
      }
      if (!data.petType) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Pet type is required when bringing pets", path: ["petType"] });
      }
      if (!data.petBreed) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Pet breed is required when bringing pets", path: ["petBreed"] });
      }
      if (!data.petAge) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Pet age is required when bringing pets", path: ["petAge"] });
      }
      if (!data.petVaccinationDate) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Pet vaccination date is required when bringing pets", path: ["petVaccinationDate"] });
      }
      if (!data.petVaccination) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Pet vaccination record image is required when bringing pets", path: ["petVaccination"] });
      }
      if (!data.petImage) {
        ctx.addIssue({ code: z.ZodIssueCode.custom, message: "Pet image is required when bringing pets", path: ["petImage"] });
      }
    }
