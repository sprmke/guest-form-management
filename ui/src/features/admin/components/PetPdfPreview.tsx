...
export function PetPdfPreview({ details, signatureUrl, className }: PetPdfPreviewProps) {
  ...
  React.useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void (async () => {
        ...
        try {
          const bytes = await renderPetPdfPreview(details, signatureUrl);
  ...
  }, [details.gafUnitOwner, details.gafTowerAndUnitNumber, signatureUrl]);
                ...
                >
                  {pageImages.map((src, index) => (
                    <img
                      key={index}
                      src={src}
                      alt={`Pet form preview page ${index + 1}`}
                      draggable={false}
