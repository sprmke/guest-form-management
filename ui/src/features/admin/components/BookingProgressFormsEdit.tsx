...
export function BookingProgressFormsEdit({
  ...
  const emit = (next: Partial<ProgressFormEditState> = {}) => {
    const state: ProgressFormEditState = {
      pricing: showPricing
        ? (next.pricing !== undefined ? next.pricing : pricing)
        : null,
      parking: showParking
        ? (next.parking !== undefined ? next.parking : parking)
        : null,
      guestBalance: showGuestBalance
        ? (next.guestBalance !== undefined ? next.guestBalance : guestBalance)
        : null,
      sdRefundGuest: showSdRefundGuest
        ? (next.sdRefundGuest !== undefined ? next.sdRefundGuest : sdRefundGuest)
        : null,
      sdSettlement: showSdSettlement
        ? (next.sdSettlement !== undefined ? next.sdSettlement : sdSettlement)
        : null,
    };
    onStateChange(state);
  };
