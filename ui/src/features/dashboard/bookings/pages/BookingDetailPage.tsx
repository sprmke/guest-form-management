...
} from '@/features/dashboard/bookings/hooks/useReceiptAiBackfill';
import {
              ...
              >
                ...
                >
                  ...
                  {editMode ? (
                    ...
                    <>
                      <GuestInfoCard booking={booking} onPreview={handlePreview} />
                      <GuestsCard
                        booking={booking}
                        onPreview={handlePreview}
                      ...
                      {booking.need_parking && (
                        <ParkingCard booking={booking} onPreview={handlePreview} />
                      )}
                      {booking.has_pets && <PetsCard booking={booking} onPreview={handlePreview} />}
                      <OtherInfoCard booking={booking} />
                      <DocumentsCard booking={booking} onPreview={handlePreview} />
                    </>
                  )}
    ...
    <div className={cn('border-border bg-card rounded-xl border p-5 shadow-sm sm:p-6', className)}>
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between md:gap-6">
        <div className="min-w-0 space-y-2">
          <h1 className="text-foreground text-base font-bold leading-snug sm:text-[17px]">
            {heading}
          </h1>
          {showPrimarySubtitle && (
          ...
          <div className="text-ui text-muted-foreground flex flex-wrap items-center gap-x-4 gap-y-2">
        ...
        <div className="flex flex-wrap items-center gap-2 md:shrink-0 md:justify-end">
          ...
          {editMode ? (
            <button
              ...
              className="border-border bg-card text-muted-foreground hover:border-border hover:bg-muted/50 inline-flex min-h-[44px] items-center justify-center gap-1.5 rounded-xl border px-4 text-xs font-medium shadow-sm transition-colors"
            >
...
function GuestInfoCard({
  ...
  return (
    <Card title="Guest Information" icon={<User className="size-3.5" />}>
      <Grid2>
        <InfoField label="Facebook / Airbnb Name" value={booking.guest_facebook_name} />
      ...
      <div className="space-y-3">
        {visibleSlots.map((slot) => {
          ...
          return (
            <section
              key={slot.index}
              className="border-border/70 bg-muted/15 space-y-3 rounded-xl border p-4"
            >
              <div className="flex items-center gap-2">
                <div className="bg-primary/10 text-primary flex size-8 shrink-0 items-center justify-center rounded-lg">
                  <UserRound className="size-4" aria-hidden />
                </div>
                <p className="text-foreground/80 text-xs font-bold uppercase tracking-wider">
                  {slot.index}. {slot.label} Guest
                </p>
              </div>
              <Grid2>
                <InfoField label="Name" value={name} />
                <InfoField label="Age" value={age != null ? String(age) : undefined} />
              </Grid2>
              {showValidId && (
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3">
                  {validIdUrl ? (
                    ...
                    <div className="border-border/70 bg-card/80 text-muted-foreground flex min-h-[44px] items-center rounded-xl border border-dashed px-3 text-xs">
  ...
  return (
    <Card title="Parking" icon={<Car className="size-3.5" />}>
      <Grid2>
        ...
        <InfoField
          label="Vehicle"
          value={[booking.car_brand_model, booking.car_color].filter(Boolean).join(' · ')}
        />
        {booking.parking_owner?.trim() ? (
          <InfoField label="Parking owner / agent" value={booking.parking_owner} />
        ) : null}
    ...
    <Card title="Pet Information" icon={<Dog className="size-3.5" />}>
      <Grid2>
        ...
        {booking.pet_fee != null && (
          <InfoField label="Pet Fee" value={formatMoney(booking.pet_fee as number)} />
        )}
      </Grid2>
...
function OtherInfoCard({ booking }: { booking: BookingRow }) {
  ...
  return (
    <Card title="Other Information" icon={<Info className="size-3.5" />}>
      {/* Booking Source */}
      <div className="mb-3">
        <p className="text-overline mb-1">Booking Source</p>
        <span
          className={`inline-flex items-center gap-1.5 rounded-xl border px-3 py-1 text-xs font-medium ${
            isAirbnb
              ...
              : 'border-primary/25 bg-primary/5 text-primary dark:border-primary/30 dark:bg-primary/15 dark:text-primary'
          }`}
        >
          {source}
        </span>
      ...
      <div className="mb-3">
        <p className="text-overline mb-1">Surprise decor</p>
        <span
      ...
      {(booking.find_us || booking.find_us_details) && (
        <div className="mb-3">
          <p className="text-overline mb-1">How they found us</p>
          <div className="flex flex-wrap gap-2">
            {booking.find_us && (
              <span className="border-border bg-muted/50 text-foreground flex items-center gap-1.5 rounded-xl border px-3 py-1 text-xs">
                <Search className="text-muted-foreground size-3" />
                {booking.find_us}
              </span>
            )}
            {booking.find_us_details && (
              <span className="text-muted-foreground py-1 text-xs">{booking.find_us_details}</span>
            )}
          </div>
        </div>
      )}
      {booking.guest_special_requests && (
        <div>
          <p className="text-overline mb-1">Special Requests</p>
...
function PricingSummaryCard({
  ...
  return (
    <Card title="Pricing" icon={<Banknote className="size-3.5" />}>
      ...
      {isCompleted && booking.next_stay_voucher_code && <NextStayVoucherCard booking={booking} />}

      {isCompleted && booking.sd_refund_receipt_url && (
        <div className="border-separator mt-4 border-t pt-4">
...
function NextStayVoucherCard({ booking }: { booking: BookingRow }) {
  ...
  return (
    <div className="border-separator mt-4 border-t pt-4">
      ...
      <div className="dark:via-card relative overflow-hidden rounded-xl border border-emerald-200/80 bg-gradient-to-br from-emerald-50 via-white to-emerald-50/60 px-4 py-3 ring-1 ring-emerald-100/80 dark:border-emerald-500/30 dark:from-emerald-950/50 dark:to-emerald-900/25 dark:ring-emerald-500/20">
        <Sparkles
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0 flex-1">
            ...
            <p className="text-caption mt-0.5 text-emerald-900/70 dark:text-emerald-200/80">
  ...
  return (
    <Card title="Documents" icon={<FileText className="size-3.5" />}>
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {docs.map((d) => (
          <DocPreview key={d.label} label={d.label} url={d.url} onPreview={onPreview} />
        ))}
      </div>
    </Card>
  );
