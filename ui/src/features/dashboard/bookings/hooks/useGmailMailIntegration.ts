...
export function useGmailMailIntegrationStatus(options?: { enabled?: boolean }) {
  const propertyId = usePropertyIdParam();
  const enabled = options?.enabled ?? true;

  return useQuery({
    queryKey: ['gmail-mail-integration', propertyId],
    enabled,
    queryFn: async (): Promise<GmailMailIntegrationStatus> => {
      const jwt = await getAdminJwt();
      const res = await fetch(scopedFunctionsUrl('/google-mail-oauth-status', propertyId), {
...
export function useStartGmailMailOAuth() {
  ...
  return useMutation({
    mutationFn: async () => {
      ...
      const res = await fetch(scopedFunctionsUrl('/google-mail-oauth-start', propertyId), {
        method: 'POST',
        headers: {
...
export function useDisconnectGmailMail() {
  ...
  return useMutation({
    mutationFn: async () => {
      const jwt = await getAdminJwt();
      const res = await fetch(scopedFunctionsUrl('/google-mail-oauth-disconnect', propertyId), {
        method: 'POST',
        headers: { Authorization: `Bearer ${jwt}` },
      });
    ...
    onSuccess: async () => {
      await qc.invalidateQueries({ queryKey: ['gmail-mail-integration', propertyId] });
    },
  });
}
