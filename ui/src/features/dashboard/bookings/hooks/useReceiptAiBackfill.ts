...
import { showDocumentAiModelErrorToast } from '@/features/dashboard/bookings/components/ReceiptAiVerdictBadge';
...
function bookingNeedsReceiptAiBackfill(
  ...
  if (documentUrlNeedsAiBackfill(booking.payment_receipt_url, booking.dp_receipt_ai_verdict)) {
    return true;
  }
  if (
  ...
  if (documentUrlNeedsAiBackfill(booking.valid_id_url, booking.valid_id_ai_verdict)) {
    return true;
  }
  return false;
}
...
export function useReceiptAiBackfill(booking: BookingRow | null | undefined) {
  ...
  const mutation = useMutation({
    mutationFn: async (bookingId: string) => {
      ...
      const res = await fetch(scopedFunctionsUrl('/validate-booking-receipts', propertyId), {
        method: 'POST',
        headers: {
    ...
    onError: (err) => {
      attemptedForId.current = null;
      showDocumentAiModelErrorToast(err instanceof Error ? err.message : String(err));
    },
  });
