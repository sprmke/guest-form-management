...
export function BookingGuestDocReplacer({
  ...
  const displayUrl = currentUrl ? (normalizeStoragePublicUrl(currentUrl) ?? currentUrl) : '';
  ...
  return (
    <div className="border-border/60 bg-muted/20 ring-border/20 flex flex-col gap-2.5 rounded-xl border p-3.5 ring-1">
      ...
      {currentUrl ? (
          ...
          preview={
            ...
            >
              ...
              <div className="min-w-0 flex-1">
                <p className="text-muted-foreground truncate text-xs font-medium">Current file</p>
                <p className="text-primary dark:text-primary flex items-center gap-0.5 text-[11px] group-hover:underline">
                  View
                </p>
              </div>
            </button>
        ...
        <div className="border-border/70 bg-card/80 text-muted-foreground flex h-16 items-center justify-center rounded-lg border border-dashed text-xs">
          No file uploaded
        </div>
      )}
