...
export function StayFinanceDrawer({ row, onClose }: Props) {
  ...
  return (
    <>
      ...
      />
      <div
        className="fixed inset-x-0 bottom-0 z-50 max-h-[min(90dvh,32rem)] overflow-y-auto rounded-t-2xl border-t border-slate-200 bg-white shadow-2xl sm:inset-x-auto sm:right-0 sm:top-0 sm:h-full sm:max-h-none sm:w-[min(100vw-1rem,24rem)] sm:rounded-none sm:rounded-l-2xl sm:border-l sm:border-t-0"
        role="dialog"
        aria-labelledby="stay-finance-title"
      >
        {/* Header */}
        <div className="sticky top-0 z-10 flex items-start justify-between gap-2 border-b border-slate-100 bg-white px-5 py-4">
          <div className="min-w-0">
            ...
            <p className="mt-0.5 text-xs text-slate-500">
              {formatBookingDate(row.check_in_date)} – {formatBookingDate(row.check_out_date)}
            </p>
            <div className="mt-2">
          ...
          <button
            type="button"
            className="flex min-h-[36px] min-w-[36px] shrink-0 items-center justify-center rounded-lg text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-600"
            onClick={onClose}
            aria-label="Close"
          >
            <X className="size-5" />
        ...
        <div className="space-y-5 p-5">
          {/* Guest balance section */}
          <Section label="Guest balance" icon={Wallet}>
            <DetailRow label="Total due" value={formatMoney(fin.totalGuestBalance)} />
            <DetailRow label="Collected" value={formatMoney(fin.guestCollected)} highlight="positive" />
            <DetailRow label="Unpaid" value={formatMoney(fin.guestUnpaid)} highlight={fin.guestUnpaid != null && fin.guestUnpaid > 0 ? 'warning' : undefined} />
          </Section>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={cn(
                        ...
                        fin.hostNet >= 0 ? 'text-emerald-700' : 'text-red-600',
                      )}
                    >
        ...
        <div className="sticky bottom-0 border-t border-slate-100 bg-white p-4">
          <Link
            to={`/bookings/${row.id}`}
            className="flex min-h-[44px] w-full items-center justify-center gap-2 rounded-xl bg-teal-700 text-sm font-semibold text-white shadow-sm transition-colors hover:bg-teal-800"
          >
...
function DetailRow({
  ...
  return (
    <div className="flex items-center justify-between gap-3 rounded-md px-2 py-2 transition-colors hover:bg-slate-50">
