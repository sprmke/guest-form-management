...
3. Confirm the matching files below were updated, or explicitly note why the change is docs-exempt (typo / rename-only).
...
| **Pricing plans / tier catalog / `planPresentation` / entitlements** | **`/for-hosts/pricing`** (live via `list-public-pricing-plans` + shared `planPresentation.ts`) **and** **`docs/architecture/plans-feature-matrix.md`** + host Plans route guides (`org/property/plans.md`, `org/plans.md`, `for-hosts.md`)                                                                                      |
...
Trivial fixes (typos, internal renames with no behavior/API/UX impact) don't need doc updates — everything else does. Doc index: `docs/README.md`. Full rules/skills index: `.cursor/rules/README.md` · Claude: `.claude/README.md`. Skills: `documentation-maintenance`, `docs-first`, `route-guides`.
